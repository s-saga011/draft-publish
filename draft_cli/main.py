...
def get_base_url():
    ...
    if not url:
        typer.echo("サーバー未設定。`draft remote <URL>` で設定してください。")
        raise typer.Exit(1)
    return url
...
def ssh_authenticate(base_url):
    ...
    typer.echo(f"認証失敗。Web UIでSSH鍵を登録してください。")
    raise typer.Exit(1)
...
@app.command()
def remote(url: str = typer.Argument(..., help="サーバーURL")):
    """サーバーURLを設定"""
    try: httpx.get(f"{url}/api/articles/?limit=1", timeout=5)
    except httpx.ConnectError:
        typer.echo(f"接続失敗: {url}"); raise typer.Exit(1)
    config = get_config()
...
"""
    ...
    console.print(f"   ├── article.md    ← 編集するファイル")
    console.print(f"   └── draft.json    ← メタデータ")
    console.print(f"\n   $ cd {article_dir}")
...
def push(
    ...
    # Summary
    console.print()
    status_label = "update" if slug else "new"
    ...
    if not typer.confirm("push?", default=True):
        raise typer.Exit(0)

    # Push
    payload = {"title": a_title, "price": a_price, "tags": a_tags, "markdown_content": body}
...
def publish():
    ...
    if not slug:
        console.print("❌ まず draft push してください", style="red")
        raise typer.Exit(1)
    ...
    if res.status_code == 200:
        ...
        save_draft_json(article_dir, draft_meta)
        console.print(f"🚀 公開しました: {base_url}/articles/{slug}", style="bold green")
    else:
        console.print(f"❌ {res.text}", style="red")
